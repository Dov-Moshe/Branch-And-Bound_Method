...
def BranchAndBound(T, CONSTRAINTS, VARIABLES, OBJ, MAT, RHS,
                   ...
                   binary_vars = True):
    # first candidate, second infeasible, third integer or binary, fourth fathomed
    list_status = [0,0,0,0]
    ...
    if binary_vars:
        ...
        var   = LpVariable.dicts("", VARIABLES, lowBound=0)
...
#                                 highlight = cur_index)
        ...
        if status is 'candidate':
            list_status[0] = list_status[0] + 1
        elif status is 'infeasible':
            list_status[1] = list_status[1] + 1
        elif status is 'integer' or status is 'binary':
            list_status[2] = list_status[2] + 1
        elif status is 'fathomed':
            list_status[3] = list_status[3] + 1
    ...
    print("Searching: %s" % search_strategy)
    if complete_enumeration:
        print("Complete enumeration")
    print("%s nodes visited " %node_count)
    print("%s LP's solved" %lp_count)
    print('num of candidate ', list_status[0])
    print('num of infeasible ', list_status[1])
    print('num of solution ', list_status[2])
    print('num of pruned ', list_status[3])
    print("===========================================")
